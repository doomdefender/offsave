...
st.markdown(
    ...
    <div class="retro-intro">
        ...
        Elige MP4 o MP3 y la calidad que necesitas.
    </div>
    """,
...
def mostrar_error_yt_dlp(
    ...
    if "ffmpeg is not installed" in mensaje_minusculas or (
        ...
        st.error(
            ...
            "no puede convertir a MP3 ni unir video y audio cuando el sitio entrega pistas separadas."
        )
        st.info(
    ...
    if "ffmpeg exited with code" in mensaje_minusculas:
        ...
        st.info(
            "La descarga completa ya fue simplificada. Este error solo puede "
            "aparecer al unir audio y video o al convertir el archivo. "
            "Abre el detalle técnico para identificar el formato que falló."
        )
...
if informacion and st.session_state.url_analizada == limpiar_enlace(url):
    ...
    st.markdown(
        ...
        <div class="retro-subtitle">
            Selecciona formato y calidad. La descarga siempre será completa.
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.caption(
        "Modo estable: OFFSAVE descargará el contenido completo. "
        "La selección de punto inicial y final fue retirada para evitar "
        "fallos adicionales de FFmpeg."
    )
    ...
    if preparar:
        try:
            ...
            with st.spinner(
                "Descargando el contenido completo..."
            ):
