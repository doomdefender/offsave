...
def mostrar_error_yt_dlp(
    ...
    if es_bloqueo_geografico:
        ...
        if proxy_url:
            st.warning(
                "OFFSAVE encontró un proxy configurado, pero YouTube "
...
def opciones_comunes(enlace: str | None = None) -> dict:
    ...
    if enlace and es_enlace_facebook(enlace):
        ...
        # No se fija un User-Agent manual porque debe coincidir con la huella.
...
def descargar_archivo(
    ...
    with tempfile.TemporaryDirectory() as carpeta_temporal:
        ...
        opciones = opciones_comunes(enlace)
        opciones.update(
            {
        ...
        if inicio is not None and final is not None:
            estado.caption(
                "Descarga terminada. Recortando el archivo localmente..."
            )
